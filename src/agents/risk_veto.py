...
class RiskVetoAgent:
    ...
    def analyze(self, ingested_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 1. 檢查風暴比門檻 (3:1)
        profile = risk.calculate_risk_reward(expected_gain, max_loss)
        ...
        if not profile.is_qualified:
            veto = True
            reason_msg = f"預期風暴比為 {profile.ratio:.2f}:1，未達 3.00:1 的風控安全底線 (預估漲幅: {expected_gain}%, 可容忍停損: {max_loss}%)。"
        ...
        if target_concentration > 0.30:
            veto = True
            reason_msg = f"目標標的 {target_id} 占總資產比率達 {target_concentration * 100:.2f}%，已超過單一部位 30.0% 的風控上限。"
            veto_reasons.append(reason_msg)
            objective_findings.append(f"風控攔截：{reason_msg}")
        else:
            objective_findings.append(f"部位集中度檢測通過：目標標的占比 {target_concentration * 100:.2f}%，未達 30.0% 上限。")
